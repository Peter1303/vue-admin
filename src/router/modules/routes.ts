...
const routes: RouteRecordRaw[] = [
  {
    ...
    children: [
      ...
      //   }
      // },
      // 纯菜单项：原代码同样没有 component / redirect，项目里也没有任何外链跳转处理，
      // 它只用于在侧边菜单里渲染一条「跳转外链百度」。
      // vue-router 4 的 RouteRecordRaw 是联合类型，要求必须含
      // component / components / redirect / children 之一，否则编译不过，
      // 因此这里显式断言（不改变原行为）。
      {
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        path: '/e600',
        name: 'e600',
        // 原写法指向 `packages/exception/600.vue`，该文件并不存在，
        // 每次构建路由表都会打一条 [loader] 未找到页面的报错（两条：本文件 + baseRoutes），
        // 实际一直靠 loader 的兜底渲染 routeError.vue —— 这里直接写成真实文件，渲染结果不变。

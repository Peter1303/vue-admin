/* eslint-disable */
/*
 * 说明（Vue 2 → Vue 3 + TS 迁移）
 * ------------------------------------------------------------
 * 本文件是「vendored 第三方库」：1900-2100 公历/农历互转（Jea杨 / JJonline.Cn，2014）。
 * 原文件名为 calendar.js，迁移时仅做了两件事：
 *   1) 扩展名改为 .ts（项目要求 src 下不残留 .js）；
 *   2) 为内部函数补上形参类型标注 —— 纯类型层面改动，运行时行为完全一致。
 * 另有 2 处行为等价调整，均已在下方就近注释说明原因。
 * 除上述内容外，算法与数据结构保持原样，未做任何业务性修改。
 */
/**
...
var calendar = {
  ...
  solar2lunar: function (y: number, m: number, d: number) {
    ...
    if (!y) {
      ...
      // 原写法是 parseInt(m)：m 在库的语义里是数字，但 parseInt 只接受字符串。
      // 保留 parseInt 的取整语义，显式转成字符串再解析，结果与原来完全一致。
      var objDate = new Date(y, parseInt(String(m)) - 1, d)
    }
    var i
    var leap = 0
    var temp = 0

...
// 原 data(): id: 'uuid' + uuid()
// uuid 已升到 v14，它没有 default export，必须用具名导入的 v4
const id = `uuid${uuidv4()}`

// ⚠️ G2 v3 的 .d.ts 把 Global 声明成 class，但运行时导出的是它的**实例**（单例），
// 因此类型上取不到 colors 这类实例字段。这里显式收窄，只是为了让 TS 知道 colors 存在，
// 运行期取值方式与原来完全一致。
const Global = G2.Global as unknown as { colors: string[] }
...
function renderChart() {
  ...
  chart.axis('GDP', {
    label: {
      // g2 v3 把 axis label 的 formatter 声明为 (text: string, item: any, index: number) => string，
      // 传进来的已经是「标签文本」。原代码写的是 `(value) => (value / 1000).toFixed(0) + 'k'`，
      // 依赖 JS 的字符串→数字隐式转换完成运算；这里显式 Number(text)，结果与原来一致。
  ...
  chart
    ...
    .color('continent', (val: string) => colorMap[val])
    .shape('circle')
